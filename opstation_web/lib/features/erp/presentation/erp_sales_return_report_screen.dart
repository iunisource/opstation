...
class ErpSalesReturnReportScreen extends ConsumerStatefulWidget {
  const ErpSalesReturnReportScreen({super.key});
  @override
  ConsumerState<ErpSalesReturnReportScreen> createState() => _ErpSalesReturnReportScreenState();
}

class _ErpSalesReturnReportScreenState extends ConsumerState<ErpSalesReturnReportScreen> {
  bool _loading = true;
  bool _running = false;
  bool _ran = false;
  String? _orgId;
  ...
  Future<void> _run() async {
    ...
    try {
      // ───────── Sales Return (credit) Invoices ─────────
      if (_source == 'both' || _source == 'invoice') {
        var q = c
            .from('sales_return_invoices')
        ...
        for (final part in _chunk(keptIds, 300)) {
          ...
          final items = await c
              .from('sales_return_invoice_items')
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (!_ran) ...[
          ...
          const Text('Returns across credit notes and POS for any period',
    ...
    return Container(
      ...
      child: Wrap(spacing: 12, runSpacing: 12, crossAxisAlignment: WrapCrossAlignment.center, children: [
        ...
        _dropdown('Source', _source, const {
          'both': 'Credit notes + POS',
          'invoice': 'Credit notes only',
          'pos': 'POS returns only',
      ...
      Row(children: [
        Expanded(
          child: Wrap(spacing: 18, runSpacing: 4, children: [
            ...
            _kv('Total returns', _money.format(_grandAmount)),
          ]),
        ),
        OutlinedButton.icon(
  ...
  Future<void> _printPdf() async {
    ...
    final srcLabel = {'both': 'Credit notes + POS', 'invoice': 'Credit notes', 'pos': 'POS returns'}[_source];
    ...
    doc.addPage(pw.MultiPage(
      ...
      build: (ctx) => [
        ...
        pw.Text('Sales Return Report - ${productMode ? 'Product-wise' : 'Customer-wise'}',
        ...
        pw.Text('Total returns: ${_money.format(totalAmt)}',
