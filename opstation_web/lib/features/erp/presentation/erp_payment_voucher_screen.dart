...
class _ErpPaymentVoucherScreenState extends ConsumerState<ErpPaymentVoucherScreen> {
  ...
  List<_VLine> _lines = [];
  // Total lives on a notifier so typing an amount rebuilds ONLY the total text,
  // not the whole voucher — keeps long vouchers fast.
        ...
        Expanded(child: _loadingMaster
          ...
          : ListView.separated(
              ...
              itemBuilder: (_, i) => _LineWidget(
                ...
                onChanged: () { setState(() {}); _refreshTotal(); }, // account pick — infrequent
                onAmountChanged: _refreshTotal,                      // per keystroke — total only
              ),
            )),
        // Footer
