    ...
    try {
      ...
      switch (type) {
        case 'Purchase Invoice':
          voucher = await client.from('purchase_invoices').select('*, suppliers(shop_name, code)').eq('id', id).maybeSingle();
          if (voucher != null) lines = await client.from('purchase_invoice_items').select('*, products(name, sku)').eq('invoice_id', id);
          break;
        case 'Purchase Return':
          voucher = await client.from('purchase_return_invoices').select('*, suppliers(shop_name, code)').eq('id', id).maybeSingle();
