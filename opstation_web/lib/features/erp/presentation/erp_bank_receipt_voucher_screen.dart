...
///
/// A BRV is MEMO-ONLY: pending cheques write NO general-ledger entry, so they
/// don't touch the customer ledger balance or aging. Each cheque line clears
/// independently. Clearing a line spawns a *posted* CRV (deposit bank + one
/// customer line) which carries the actual GL (Dr Bank / Cr 1210) — mirroring
/// the Receipt Vouchers screen exactly, so there's a single, consistent posting
/// shape and no double-posting (each spawned CRV has its own unique id).
class ErpBankReceiptVoucherScreen extends ConsumerStatefulWidget {
  const ErpBankReceiptVoucherScreen({super.key});
  @override
  ConsumerState<ErpBankReceiptVoucherScreen> createState() =>
      _ErpBankReceiptVoucherScreenState();
}
...
class _ErpBankReceiptVoucherScreenState
  ...
  bool get _isSavedVoucher => _currentVoucher != null;
  ...
  /// Leaf Current-Assets accounts (BANK / CASH) — the deposit targets, same
  /// definition the Receipt Vouchers screen uses for its cash account picker.
  ...
  Future<void> _loadVoucher(Map<String, dynamic> v) async {
    try {
      final rows = await Supabase.instance.client
          .from('bank_receipt_voucher_lines')
          .select()
      ...
      setState(() {
        ...
        _customerName = v['customer_name'] as String? ?? '';
  ...
  void _addLine() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) nl.descFocus.requestFocus();
    });
  }

  void _removeLine(int i) {
    final l = _lines[i];
    if (l.status != 'pending') return; // can't remove cleared/bounced
    setState(() {
      if (!l.id.startsWith('brvl_new_')) _deletedLineIds.add(l.id);
      l.dispose();
      _lines.removeAt(i);
    });
  ...
  Future<void> _save() async {
    ...
    final valid =
        _lines.where((l) => (double.tryParse(l.amtCtrl.text) ?? 0) > 0).toList();
    if (valid.isEmpty) {
      _snack('Add at least one cheque with an amount');
      return;
    }
    final orgId = _orgId;
    final bid = _branchId;
    ...
    try {
      ...
      final total =
          valid.fold<double>(0, (s, l) => s + (double.tryParse(l.amtCtrl.text) ?? 0));
      ...
      // Remove deleted (pending) lines.
      for (final delId in _deletedLineIds) {
        await client.from('bank_receipt_voucher_lines').delete().eq('id', delId);
      }
      _deletedLineIds.clear();

      // Upsert each cheque line. Cleared/bounced lines are written back
      // unchanged (their status/links preserved); pending lines are saved fresh.
      for (var i = 0; i < _lines.length; i++) {
        final l = _lines[i];
        final amt = double.tryParse(l.amtCtrl.text) ?? 0;
        if (amt <= 0 && l.status == 'pending') continue; // skip empty pending
        final lineId = l.id.startsWith('brvl_new_')
            ? 'brvl_${DateTime.now().microsecondsSinceEpoch}_$i'
            : l.id;
        l.id = lineId;
        await client.from('bank_receipt_voucher_lines').upsert({
          'id': lineId,
          'voucher_id': vid,
          'org_id': orgId,
          'line_order': i,
  ...
  Future<void> _clearLine(int index) async {
    ...
    // Pick deposit account + clearance date.
    String? bankId;
    String bankName = '';
        ...
        builder: (dCtx, setS) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                Text('Amount: Rs. ${_fmt.format(amt)}',
    ...
    try {
      ...
      await client.from('crv_voucher_lines').insert({
        ...
        'account_id': _customerId,
        'account_name': _customerName,
        'description': 'Cheque clearance — $brvNum',
        'amount': amt,
        'line_order': 0,
      });

      // 2) Post GL — mirrors _postCrvToGL in the Receipt Vouchers screen.
      ...
      await client.from('bank_receipt_voucher_lines').update({
        'status': 'cleared',
  ...
  Future<void> _bounceLine(int index) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
            'This marks the cheque as bounced. No ledger or aging impact (it was never posted).'),
        actions: [
          TextButton(
  ...
  void _print() {
    ...
    final rows = _lines
        .where((l) => (double.tryParse(l.amtCtrl.text) ?? 0) > 0)
        .toList();
    final body = rows.asMap().entries.map((e) {
      ...
      final cd = l.chequeDate != null
          ...
          : '-';
      final amt = double.tryParse(l.amtCtrl.text) ?? 0;
      return '<tr><td>${e.key + 1}</td><td>${l.descCtrl.text}</td><td>$cd</td>'
          '<td style="text-transform:capitalize">${l.status}</td>'
          '<td style="text-align:right">${_fmt.format(amt)}</td></tr>';
    }).join();
    final html_str =
        '''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Bank Receipt Voucher</title><style>@page{margin:0}
    ...
    <h2>Bank Receipt Voucher (Post-Dated Cheques)</h2>
    <table style="border:none;margin-bottom:5px;width:100%"><tr>
      ...
      <td style="border:none;padding:1px 10px;font-size:10px"><b>Customer:</b> $_customerName</td>
    ...
    <table><thead><tr><th>#</th><th>Description</th><th>Cheque Date</th><th>Status</th><th style="text-align:right">Amount (Rs.)</th></tr></thead><tbody>
    $body
    </tbody><tfoot><tr><td colspan="4" class="total" style="text-align:right">Total:</td><td class="total" style="text-align:right">Rs. ${_fmt.format(_total)}</td></tr></tfoot></table>
                                ...
                                child: Container(
                                  ...
                                  child: Column(
                                      ...
                                      children: [
                                        ...
                                        Text(v['customer_name'] as String? ?? '',
                              ...
                              onSelect: (c) {
                                ...
                                if (_lines.isNotEmpty) {
                                  WidgetsBinding.instance
                                    ...
                                    if (mounted) {
                                      _lines.first.descFocus.requestFocus();
                                    }
                                  });
                                }
                              },
              ...
              child: _loadingMaster
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (_, i) => _ChequeLineRow(
                        key: ValueKey('brvline_${_lines[i].id}'),
                        line: _lines[i],
                        lineNum: i + 1,
                        fmt: _fmt,
                        onPickDate: () async {
                        ...
                        onAmountSubmitted: () {
                          if (i == _lines.length - 1) {
                            ...
                            _lines[i + 1].descFocus.requestFocus();
                          }
                        },
            ...
            Container(
              ...
              child: Row(children: [
                TextButton.icon(
                    ...
                    onPressed: _addLine),
                const Spacer(),
                Text('${_lines.where((l) => (double.tryParse(l.amtCtrl.text) ?? 0) > 0).length} cheques',
                    style: const TextStyle(
                        fontSize: 12, color: AppTheme.textSecondary)),
...
class _ChequeLine {
  ...
  _ChequeLine({String? id, this.status = 'pending'})
      : id = id ?? 'brvl_new_${DateTime.now().microsecondsSinceEpoch}';
...
class _ChequeLineRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final dateStr = line.chequeDate != null
        ...
        : 'Pick date';
    Color stColor = line.status == 'cleared'
        ? Colors.green
        : line.status == 'bounced'
            ? AppTheme.danger
            : Colors.orange;
...
// ── Customer dropdown: type to filter, Enter selects top match ────────────────
class _CustomerDropdown extends StatefulWidget {
  final String value;
  final List<Map<String, dynamic>> customers;
  final ValueChanged<Map<String, dynamic>> onSelect;
  const _CustomerDropdown(
      {required this.value, required this.customers, required this.onSelect});
  @override
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      InkWell(
        ...
        child: Container(
          ...
          child: Row(children: [
            Expanded(
                child: Text(widget.value.isNotEmpty ? widget.value : 'Select customer',
                    style: TextStyle(
                        fontSize: 13,
...
// ── Searchable list (deposit account picker inside the clear dialog) ──────────
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      InkWell(
        ...
        child: Container(
          ...
          child: Row(children: [
            Expanded(
                child: Text(widget.value.isNotEmpty ? widget.value : widget.hint,
                    style: TextStyle(
                        fontSize: 13,
                        color: widget.value.isEmpty
                            ? Colors.grey
