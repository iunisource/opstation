...
class _ErpSupplierAgingScreenState extends ConsumerState<ErpSupplierAgingScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      // table is `payment_vouchers` (per existing code).
      var rq = client.from('payment_vouchers')
          .select('supplier_id, amount, voucher_date')
          .eq('org_id', orgId);
      ...
      for (final r in receipts as List) {
        final cid = (r as Map)['supplier_id'] as String?;
        if (cid == null) continue;
        paidBySupplier[cid] = (paidBySupplier[cid] ?? 0) + ((r['amount'] as num?)?.toDouble() ?? 0);
      }
    ...
    doc.addPage(pw.MultiPage(
      ...
      build: (ctx) => [
        ...
        pw.Table.fromTextArray(
          headers: ['#', 'Supplier', 'Code', 'Invoices', '0–30', '31–60', '61–90', '91+', 'Total'],
          data: [
