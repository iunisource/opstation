...
class _ErpStockBalanceReportScreenState extends ConsumerState<ErpStockBalanceReportScreen> {
  ...
  double get _totalQty => _list.fold(0.0, (s, r) => s + _qty(r));
  ...
  void _print() {
    ...
    final htmlStr = '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Stock Balance Report</title>'
        ...
        '<div class="muted">Generated by ${esc(userName)} &middot; $gen &middot; ${list.length} item(s)</div>'
        '<table><thead><tr>'
        '<th>Product</th><th>SKU</th><th>UOM</th><th style="text-align:right">On Hand</th>'
        '</tr></thead><tbody>$body</tbody>'
        '<tfoot><tr>'
        '<td colspan="3" style="text-align:right;font-weight:bold;background:#f4f5f7">Total</td>'
        '<td style="text-align:right;font-weight:bold;background:#f4f5f7">${_qtyStr(_totalQty)}</td>'
        '</tr></tfoot></table>'
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text('On-hand quantity as of ${_fmtDisplay(_asOf)}. ${list.length} item${list.length == 1 ? '' : 's'}. Generated by ${ref.read(currentUserProvider)?.name ?? '-'}.',
            ...
            : Container(
                ...
                child: Column(children: [
                  ...
                  if (list.isNotEmpty) Container(
                    ...
                    child: Row(children: [
                      const Expanded(flex: 4, child: Text('Total', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 13))),
                      const Expanded(flex: 2, child: SizedBox()),
                      const Expanded(flex: 1, child: SizedBox()),
                      Expanded(flex: 2, child: Text(_qtyStr(_totalQty), textAlign: TextAlign.right, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 13))),
                    ]),
                  ),
                ]),
              )),
