...
class _State extends ConsumerState<ErpDamageStockVoucherScreen> {
  ...
  Future<String?> _save({bool silent = false}) async {
    ...
    try {
      ...
      if (_current == null) {
        final ex = await client.from('damage_vouchers').select('voucher_number').eq('org_id', orgId)
            .like('voucher_number', 'DMG-${_date.year}-%');
        int mx = 0;
        for (final r in (ex as List)) {
          final n = int.tryParse((r['voucher_number'] as String? ?? '').split('-').last) ?? 0;
          if (n > mx) mx = n;
        }
        num = 'DMG-${_date.year}-' + (mx + 1).toString().padLeft(4, '0');
