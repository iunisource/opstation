...
class _ErpPurchaseReturnsScreenState extends ConsumerState<ErpPurchaseReturnsScreen> {
  ...
  Future<void> _loadList() async {
    ...
    try {
      var q = Supabase.instance.client.from('purchase_returns')
          .select('id, voucher_number, voucher_date, grand_total, status, suppliers(name), sales_invoices(voucher_number)')
          .eq('org_id', orgId);
      ...
      // ignore: avoid_print
      print('[PurchaseReturns] load list error: $e');
  ...
  Future<void> _loadDetail(String id) async {
    ...
    try {
      ...
      final ret = await client.from('purchase_returns')
          .select('*, suppliers(name, address, contact_person, contact_number), sales_invoices(voucher_number), branches(name)')
          .eq('id', id).single();
      ...
      // ignore: avoid_print
      print('[PurchaseReturns] load detail error: $e');
  ...
  Future<void> _createNew() async {
    ...
    final invs = await Supabase.instance.client.from('purchase_invoices')
    ...
    if (invList.isEmpty) { _showSnack('No purchase invoices to return against'); return; }

    final pi = await showDialog<Map<String, dynamic>>(
      context: context,
      builder: (ctx) => _PiPicker(invoices: invList),
    );
    if (pi == null) return;

    setState(() => _detailLoading = true);
    try {
      final piItems = await Supabase.instance.client.from('purchase_invoice_items')
          .select('*, products(name, sku), uoms(abbreviation)').eq('invoice_id', pi['id']);
      if (!mounted) return;
      setState(() => _detailLoading = false);
      ...
      final returnLines = await showDialog<List<Map<String, dynamic>>>(
        context: context,
        builder: (_) => _ReturnItemsDialog(sourceItems: List<Map<String, dynamic>>.from(piItems), title: 'Items to Return'),
      );
      ...
      for (final l in returnLines) {
        ...
        await Supabase.instance.client.from('purchase_return_items').insert({
          'id': 'pri_${DateTime.now().microsecondsSinceEpoch}_${pid.substring(0, 4)}',
          'return_id': retId,
        ...
        // Stock: subtract qty (purchase return = goods leaving warehouse back to supplier)
        ...
        await Supabase.instance.client.from('inventory_movements').insert({
          ...
          'reference_id': retId, 'reference_type': 'purchase_return',
  ...
  Future<void> _delete() async {
    ...
    try {
      // Reverse: add stock back (delete cancels the purchase return)
      for (final item in _items) {
        ...
        await Supabase.instance.client.from('inventory_movements').insert({
          ...
          'quantity': qty, 'movement_type': 'adjustment',
          'reference_id': _detail['id'], 'reference_type': 'purchase_return_deleted',
          'moved_at': DateTime.now().toUtc().toIso8601String(), 'created_by': userId,
        });
      }
  ...
  Future<void> _print() async {
    ...
    final piVoucher = _detail['purchase_invoices']?['voucher_number'] as String?;
    await VoucherPdf.printVoucher(
      ...
      customerPhone: sup?['contact_number'] as String?,
      ...
      relatedRefs: piVoucher != null ? {'Original PI #': piVoucher} : null,
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(color: AppTheme.background, child: Row(children: [
      ...
      Expanded(child: _selectedId == null
          ? const Center(child: Text('Select a Purchase Return', style: TextStyle(fontSize: 16, color: AppTheme.textSecondary)))
          : _buildDetail()),
    ]));
  }
  ...
  Widget _buildList() {
    ...
    final filtered = _returns.where((r) {
      ...
      return (r['voucher_number'] as String? ?? '').toLowerCase().contains(q) ||
             ...
             ((r['purchase_invoices']?['voucher_number'] as String?) ?? '').toLowerCase().contains(q);
    }).toList();
    return SizedBox(width: 360, child: Container(
      ...
      child: Column(children: [
        Padding(
          ...
          child: Row(children: [
            const Expanded(child: Text('Purchase Returns', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700))),
            IconButton(
        ...
        Padding(
          ...
          child: TextField(
            decoration: const InputDecoration(
              hintText: 'Search return / PI / supplier…',
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return ListTile(
                          ...
                          subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Text('SI: ${r['purchase_invoices']?['voucher_number'] ?? '-'}', style: const TextStyle(fontSize: 11)),
                            Text(r['suppliers']?['shop_name'] as String? ?? '-', style: const TextStyle(fontSize: 11)),
                          ]),
                          trailing: Text(
  ...
  Widget _buildDetail() {
    ...
    final pi = _detail['purchase_invoices']?['voucher_number'] as String?;
    final sup = _detail['suppliers'] as Map?;
      ...
      Container(
        ...
        child: Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const Text('Purchase Return', style: TextStyle(fontSize: 11, color: AppTheme.textSecondary, letterSpacing: 1.2)),
          ])),
            ...
            if (pi != null)
              Container(
                ...
                child: Row(children: [
                  ...
                  const Text('Original PI:', style: TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
                  const SizedBox(width: 6),
                  Text(pi, style: const TextStyle(fontWeight: FontWeight.w700, color: AppTheme.primary)),
                  const SizedBox(width: 16),
                  Text(sup?['name'] as String? ?? '-', style: const TextStyle(fontSize: 12)),
                ]),
              ),
            Container(
...
class _PiPicker extends StatefulWidget {
  final List<Map<String, dynamic>> invoices;
  const _PiPicker({required this.invoices});
  @override
  State<_PiPicker> createState() => _PiPickerState();
}

class _PiPickerState extends State<_PiPicker> {
  String _q = '';
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = widget.invoices.where((inv) {
      ...
      return (inv['voucher_number'] as String? ?? '').toLowerCase().contains(q) ||
             (inv['suppliers']?['shop_name'] as String? ?? '').toLowerCase().contains(q);
    }).toList();
    return AlertDialog(
      title: const Text('Pick Source Purchase Invoice'),
      content: SizedBox(width: 520, height: 460, child: Column(children: [
        TextField(
          decoration: const InputDecoration(hintText: 'Search PI / supplier', prefixIcon: Icon(Icons.search, size: 18), isDense: true),
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return RadioListTile<Map<String, dynamic>>(
                      ...
                      subtitle: Text(
                        '${inv['suppliers']?['shop_name'] ?? '-'} · ${(inv['grand_total'] as num?)?.toStringAsFixed(2) ?? '0'} · ${inv['voucher_date']}',
      ...
      actions: [
        ...
        ElevatedButton(onPressed: () {
          ...
          for (final it in widget.sourceItems) {
            ...
            if (q <= 0) continue;
            final originalQty = (it['quantity'] as num?)?.toDouble() ?? 0;
            if (q > originalQty) {
