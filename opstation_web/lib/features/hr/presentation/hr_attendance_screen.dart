...
class _State extends ConsumerState<HrAttendanceScreen> {
  ...
  Future<void> _generateRegister(DateTime from, DateTime to) async {
    ...
    try {
      ...
      // Scope the register to the active employee search too, so exporting
      // while a search is applied gives that one employee's (or matches')
      // attendance for the range — not the whole roster.
      final q = _search.trim();
      final emps = _employees.where((e) {
        if (eff != null && e['branch_id'] != eff) return false;
        if (q.isNotEmpty &&
            !matchesQuery('${e['full_name'] ?? ''} ${e['employee_code'] ?? e['code'] ?? ''}', q)) {
          return false;
        }
        return true;
      }).toList();
