...
class ErpOpeningJournalScreen extends ConsumerStatefulWidget {
  const ErpOpeningJournalScreen({super.key});
  @override ConsumerState<ErpOpeningJournalScreen> createState() => _State();
}
...
class _State extends ConsumerState<ErpOpeningJournalScreen> {
  ...
  Future<void> _loadMaster() async {
    ...
    try {
      ...
      final all = <Map<String,dynamic>>[
        // Postable leaves only: Level-4 detail accounts, or a Level-3 with no
      ...
      Positioned(left: pos.dx, top: pos.dy, child: Material(elevation: 8, borderRadius: BorderRadius.circular(8),
        child: IntrinsicWidth(child: Column(mainAxisSize: MainAxisSize.min, children: [
          InkWell(onTap: () {
            ...
            html.window.open(origin + '#/financials/opening-journal?id=' + num, '_blank');
  ...
  Future<void> _save({bool post = false}) async {
    ...
    try {
      ...
      _snack(post ? 'Opening Journal ' + eNum + ' posted ✓' : 'Draft saved');
      await _loadVouchers();
  ...
  Future<void> _delete() async {
    ...
    final ok = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
      title: const Text('Delete Opening Journal?'),
  ...
  void _print() {
    ...
    final postedInfo = rawPostedAt != null
        ...
        : '_______________';
    final htmlStr = '''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Opening Journal</title><style>@page{margin:0}
  ...
  @override Widget build(BuildContext context) {
    ...
    final canAdd = access?.canAddDoc('opening_jv') ?? false;
    final canEdit = access?.canEditDoc('opening_jv') ?? false;
      ...
      if (_drawerOpen) Container(width: 300,
        ...
        child: Column(children: [
          Container(padding: const EdgeInsets.fromLTRB(10,10,10,8),
            ...
            child: Column(children: [
              Row(children: [
                const Expanded(child: Text('Opening Journals', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700))),
        ...
        Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(_current?['entry_number'] as String? ?? 'New Opening Journal', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700)),
